...
export const SettingsPage: React.FC = () => {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    try {
      await changePasswordWithOldPassword(currentPassword, newPassword);
      setPassSuccess(true);
      setCurrentPassword('');
      setNewPassword('');
      setConfirmPassword('');
          ...
          <form onSubmit={handlePasswordChange} className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-black border border-zinc-800 rounded-xl text-xs text-white placeholder-zinc-500 focus:outline-none focus:border-zinc-700 font-mono"
                required
              />
            </div>
            ...
            <div className="sm:col-span-3 flex justify-end">
              <button
                ...
                className="px-5 py-2.5 bg-white hover:bg-zinc-200 text-black font-bold disabled:opacity-50 rounded-xl text-xs flex items-center gap-1.5 transition-all cursor-pointer shadow-md"
              >
                {savingPass ? (
                  <span className="animate-spin rounded-full h-3.5 w-3.5 border-2 border-black border-t-transparent" />
                ) : (
